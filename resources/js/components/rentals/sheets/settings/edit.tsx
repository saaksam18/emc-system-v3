...
import { ContactTypes, RentalStatusesType } from '@/types'; // Assuming ContactTypes type includes an 'id' field
...
// --- Main Edit Component ---
interface EditProps {
    rentalStatus: RentalStatusesType | null; // Pass the existing contact type data
...
export function Edit({ rentalStatus, onSubmitSuccess }: EditProps) {
    ...
    const handleSubmit: FormEventHandler<HTMLFormElement> = (e) => {
        ...
        const updateUrl = `/rentals/settings/deposit-type/${rentalStatus?.id}/update`;
        put(updateUrl, {
            onSuccess: () => {
                clearErrors();
            ...
            onError: (errorResponse) => {
                console.error('Error updating:', errorResponse);
            },
            preserveState: true,
            preserveScroll: true,
        });
    ...
    return (
        <div className="px-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                {/* --- Basic Information --- */}
                <FormSection title="Edit Rental Status" description="Update the details for the rental status.">
                    {/* --- Name Field --- */}
