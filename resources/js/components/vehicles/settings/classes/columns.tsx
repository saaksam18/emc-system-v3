...
import { VehicleClass } from '@/types'; // Assuming RoleObject is defined in types
...
export interface TableMeta {
    edit: (vehicle: VehicleClass) => void;
...
export const columns: ColumnDef<VehicleClass, TableMeta>[] = [
    {
        accessorKey: 'id',
        header: ({ column }) => (
    ...
    },
    {
        ...
        cell: ({ row, table }) => {
            ...
            const handleDeleteSubmit = (e: React.FormEvent<HTMLFormElement>) => {
                ...
                const toastId = toast.loading(`Deleting vehicle class ${vehicleClass.name}...`);
                destroy(route('vehicles.classes.delete', vehicleClass.id), {
                    preserveScroll: true,
                    preserveState: true,
                    data: { password: deleteData.password },
                    onSuccess: () => {
                        toast.success(`Vehicle class ${vehicleClass.name} deleted successfully.`, { id: toastId });
                        closeDeleteModal();
                    },
                    onError: (errorResponse) => {
                        ...
                        if (errorResponse.password) {
                            ...
                            toast.error(`Failed to delete vehicle class ${vehicleClass.name}. Please try again.`, { id: toastId });
                        }
                    },
                });
            };
            ...
            return (
                <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                    ...
                    <DialogContent className="sm:max-w-[425px]">
                        <DialogHeader>
                            <DialogTitle>Delete vehicle class: {vehicleClass.name}</DialogTitle>
                            <DialogDescription>
                        ...
                        <form onSubmit={handleDeleteSubmit} className="space-y-4 py-4">
                            <div className="grid gap-2">
                                <Label htmlFor={`delete-password-${vehicleClass.id}`}>Administrator Password</Label>
                                <Input
                                    id={`delete-password-${vehicleClass.id}`}
                                    type="password"
                                    name="password"
                                    ref={passwordInput}
                            ...
                            <DialogFooter className="gap-2 pt-4 sm:gap-1">
                                ...
                                <Button type="submit" variant="destructive" disabled={processingDelete || !deleteData.password}>
                                    {processingDelete ? 'Deleting...' : 'Delete Vehicle Class'}
                                </Button>
                            </DialogFooter>
                        </form>
                    </DialogContent>
