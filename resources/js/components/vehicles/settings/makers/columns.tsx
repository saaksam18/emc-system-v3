...
import { VehicleMakerType } from '@/types'; // Assuming RoleObject is defined in types
...
export interface TableMeta {
    edit: (status: VehicleMakerType) => void;
...
export const columns: ColumnDef<VehicleMakerType, TableMeta>[] = [
    {
        accessorKey: 'id',
        header: ({ column }) => (
    ...
    },
    {
        ...
        cell: ({ row, table }) => {
            const vehicleMakers = row.original;
            ...
            const handleDeleteSubmit = (e: React.FormEvent<HTMLFormElement>) => {
                ...
                const toastId = toast.loading(`Deleting vehicle maker ${vehicleMakers.name}...`);
                destroy(route('vehicles.makers.delete', vehicleMakers.id), {
                    preserveScroll: true,
                    preserveState: true,
                    data: { password: deleteData.password },
                    onSuccess: () => {
                        toast.success(`Vehicle maker ${vehicleMakers.name} deleted successfully.`, { id: toastId });
                        closeDeleteModal();
                    },
                    onError: (errorResponse) => {
                        ...
                        if (errorResponse.password) {
                            ...
                            toast.error(`Failed to delete vehicle maker ${vehicleMakers.name}. Please try again.`, { id: toastId });
                        }
                    },
                });
            };
            ...
            return (
                <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                    ...
                    <DialogContent className="sm:max-w-[425px]">
                        <DialogHeader>
                            <DialogTitle>Delete vehicle class: {vehicleMakers.name}</DialogTitle>
                            <DialogDescription>
                        ...
                        <form onSubmit={handleDeleteSubmit} className="space-y-4 py-4">
                            <div className="grid gap-2">
                                <Label htmlFor={`delete-password-${vehicleMakers.id}`}>Administrator Password</Label>
                                <Input
                                    id={`delete-password-${vehicleMakers.id}`}
                                    type="password"
                                    name="password"
                                    ref={passwordInput}
                            ...
                            <DialogFooter className="gap-2 pt-4 sm:gap-1">
                                ...
                                <Button type="submit" variant="destructive" disabled={processingDelete || !deleteData.password}>
                                    {processingDelete ? 'Deleting...' : 'Delete Vehicle Maker'}
                                </Button>
                            </DialogFooter>
                        </form>
                    </DialogContent>
