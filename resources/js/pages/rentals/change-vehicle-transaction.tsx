...
interface ChangeVehiclePageProps extends PageProps {
    flash: {
        success?: string;
        error?: string;
    ...
    availableVehicles: { id: number; vehicle_no: string }[];
    lastSale: SaleTransaction[] | undefined;
    rental: RentalsType | undefined;
}
// --- End of Type Definitions ---

function ChangeVehicleTransaction({
    vehicle,
    customers,
    vehicleStatuses,
    depositTypes,
    ...
    lastSale,
    rental,
}: ChangeVehiclePageProps) {
    const { props: pageProps } = usePage<ChangeVehiclePageProps>();
    ...
    const progressWidth = useMemo(() => {
        const totalSegments = changeVehicleSteps.length - 1;
    ...
    const nextStep = () => {
        if (validateCurrentStep()) {
            setStep((prev) => Math.min(changeVehicleSteps.length, prev + 1));
        }
    };
    const prevStep = () => {
        if (step === changeVehicleSteps.length) {
    ...
    const CurrentStepComponent = changeVehicleSteps.find((s) => s.id === step)?.component;
    const currentStepData = changeVehicleSteps.find((s) => s.id === step);
    ...
    return (
        <AppLayout breadcrumbs={updateRentalBreadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                ...
                <div className={`${step === changeVehicleSteps.length ? 'w-full' : 'grid grid-cols-1 gap-4 md:grid-cols-2'}`}>
                    ...
                    {CurrentStepComponent && currentStepData && (
                        <FormSection title={currentStepData.name} description={`Please complete the details for the ${currentStepData.name} step.`}>
                            ...
                            {step === changeVehicleSteps.length ? (
                                ''
                            ) : (
                                <div>
                ...
                <div className="w-full items-center justify-center space-y-1 space-x-1 rounded-full p-2 drop-shadow-sm md:flex md:max-w-md">
                    ...
                    {step === changeVehicleSteps.length ? (
                        ...
                        <Button
                            variant="default"
                            onClick={nextStep}
                            disabled={step === changeVehicleSteps.length}
...
export default ChangeVehicleTransaction;
